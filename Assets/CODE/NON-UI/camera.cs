...
public class camera : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (grav_Script.mouse_MODE == -1 || Input.GetKey(KeyCode.LeftControl))
        {
            zoom -= Mathf.Pow(scroll * zoomMultiplier * sprint_multiplayer, 2) * Mathf.Sign(scroll);
            zoom = Mathf.Max(zoom, 0.25f);
        }
